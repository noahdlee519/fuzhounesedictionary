...
/* One screen, one list, record-and-move-on. The point of this page is that a
   speaker with twenty minutes should get through twenty words without ever
...
export default async function RecordPage({
  ...
  let query = supabase
    .from("needs_recording")
    .select("id, headword, hanzi, romanization, short_gloss, origin_area, origin_locality, votes", {
      count: "exact",
    });
    ...
    <div className="space-y-8">
      ...
      {!error && rows.length === 0 && (
        <div className="border border-rule bg-surface p-8 text-center">
          <p className="text-inkSoft">
            {origin
              ? `Every word from ${originArea(origin)!.label} has a recording.`
              : "Every word in the dictionary has a recording. Genuinely remarkable."}
          </p>
      ...
      <ol className="divide-y divide-rule border-y border-rule">
        {rows.map((r: any, i: number) => {
          ...
          return (
            <li key={r.id} className="grid gap-3 py-5 sm:grid-cols-[1fr_auto] sm:items-start sm:gap-6">
              ...
              <Recorder userId={user.id} entryId={r.id} kind="headword" />
            </li>
          );
        })}
      </ol>
